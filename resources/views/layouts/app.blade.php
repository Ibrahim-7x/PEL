    ...
    <div class="d-flex">
        ...
        <div id="sidebar" class="vh-100 p-3 position-fixed top-0 start-0 d-flex flex-column overflow-hidden"
            ...
            <div class="text-center mb-4">
                <img src="{{ asset('images/logo abacus-1.png') }}" alt="PEL Logo" id="sidebarLogo"
                ...
                <h5 class="fw-bold text-white mb-1 sidebar-text" style="text-shadow: 0 2px 4px rgba(0,0,0,0.3);">
                    PEL-Abacus</h5>
                <hr class="mt-3 mb-0 sidebar-text"
                    style="border-top: 2px solid rgba(123, 143, 161, 0.5); width: 80%; margin: 0 auto;">
            </div>

            <!-- Sidebar Menu -->
            <ul class="nav flex-column">
                <li class="nav-item mb-2">
                    <a class="nav-link {{ request()->is('welcome') ? 'active fw-bold' : '' }}"
                        href="{{ url('/welcome') }}"
                ...
                @if (auth()->check() && strtolower(auth()->user()->role) === 'agent')
                    <li class="nav-item mb-2">
                        <a class="nav-link {{ request()->is('home-agent') ? 'active fw-bold' : '' }}"
                            href="{{ url('/home-agent') }}"
                            style="color: rgba(255,255,255,0.8); transition: all 0.3s ease; border-radius: 8px; padding: 10px 15px;">
                            <i class="bi bi-clipboard-check me-2"></i><span class="sidebar-text">RU Case
                                Registration</span>
                        </a>
                    </li>
                @endif
                ...
                @if (auth()->check() && strtolower(auth()->user()->role) === 'management')
                    <li class="nav-item mt-3 mb-1 sidebar-text">
                        <span class="text-uppercase text-white small fw-bold"
                            style="font-size: 11px; letter-spacing: 1px;">Reports</span>
                    </li>
                    <li class="nav-item mb-1">
                        <a href="{{ route('export.initial_customer') }}"
                            class="nav-link {{ request()->routeIs('export.initial_customer') ? 'active fw-bold' : '' }}"
                            style="color: rgba(255,255,255,0.8); transition: all 0.3s ease; border-radius: 8px; padding: 10px 15px;">
                    ...
                    <li class="nav-item mb-1">
                        <a href="{{ route('export.happy_call') }}"
                            class="nav-link {{ request()->routeIs('export.happy_call') ? 'active fw-bold' : '' }}"
                            style="color: rgba(255,255,255,0.8); transition: all 0.3s ease; border-radius: 8px; padding: 10px 15px;">
                    ...
                    <li class="nav-item">
                        <a href="{{ route('export.feedback') }}"
                            class="nav-link {{ request()->routeIs('export.feedback') ? 'active fw-bold' : '' }}"
                            style="color: rgba(255,255,255,0.8); transition: all 0.3s ease; border-radius: 8px; padding: 10px 15px;">
                    ...
                    <li class="nav-item">
                        <a href="{{ route('export.coms') }}"
                            class="nav-link {{ request()->routeIs('export.coms') ? 'active fw-bold' : '' }}"
                            style="color: rgba(255,255,255,0.8); transition: all 0.3s ease; border-radius: 8px; padding: 10px 15px;">
                            <i class="bi bi-chat-dots me-2"></i>
        ...
        <div class="flex-grow-1 ms-250" id="mainContent" style="transition: margin-left 0.3s;">
            <!-- Top Navbar -->
            <nav
                class="navbar navbar-light bg-light border-bottom px-3 d-flex justify-content-between align-items-center sticky-top">
